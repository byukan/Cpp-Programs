...
template<typename T, typename CMP>
TreeNode<T, CMP>::TreeNode() : data(0), left(nullptr), right(nullptr), parent(nullptr) {
	//id = generate_id();
	//creation_text();
}
...
public:
	...
	// deep copy of BST
	BinarySearchTree<T, CMP> deep_copy(const BinarySearchTree<T, CMP>& original_tree){ return BinarySearchTree<T, CMP>(original_tree); };

	BinarySearchTree<T, CMP>(const BinarySearchTree<T, CMP>& original_tree){
	...
	friend void swap(BinarySearchTree<T, CMP>& first, BinarySearchTree<T, CMP>& second); // nothrow
...
*/
template<typename T, typename CMP>
void swap(BinarySearchTree<T, CMP>& first, BinarySearchTree<T, CMP>& second){
